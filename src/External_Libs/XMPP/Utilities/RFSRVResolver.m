...
- (void)stop
{
    ...
    [self.timeoutTimer invalidate]; //TODO: need to check why it's crashing here!
...
// is done in the -_processRecord:length: method.
{
    ...
    DDLogDns_Srv(@"%s",__PRETTY_FUNCTION__);
...
- (void)_start
{
	...
	DDLogDns_Srv(@"%s Looking up %@",__PRETTY_FUNCTION__,srvName);	
...
- (void)sortResults
{
    ...
    DDLogDns_Srv(@"%s Sorted results: %@",__PRETTY_FUNCTION__, self.results);
}
